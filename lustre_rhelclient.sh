...
install_lustre_rhel7()
{
	...
	wget https://downloads.hpdd.intel.com/public/lustre/lustre-2.10.2/el7.4.1708/client/SRPMS/lustre-2.10.2-1.src.rpm
	...
	yum install -y kernel-devel kernel-debug kernel-tools
	...
	rpmbuild --define "_topdir /root/rpmbuild" --rebuild --without servers lustre-2.10.2-1.src.rpm
	
	# Install the compiled RPM whose file names are based on the currently running kernel but with - replaced by _ (e.g. )
	cd /root/rpmbuild/RPMS/x86_64/
	#yum --nogpgcheck localinstall -y lustre-client-2.1.0-$(uname -r | sed 's/-/_/').x86_64.rpm lustre-client-modules-2.7.0-$(uname -r | sed 's/-/_/').x86_64.rpm
	yum --nogpgcheck localinstall -y lustre-client-2.10.2-1.el7.x86_64.rpm kmod-lustre-client-2.10.2-1.el7.x86_64.rpm
	...
	sed "/\[main\]/a ${exclude}" -i /etc/yum.conf
